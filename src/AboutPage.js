...
const AboutPage = () => {
    ...
    const groups = "groups=~%28~%27infrastructure~%27database~%27finance~%27library~%27mobile~%27place~%27programming~%27software~%27tool~%27writing%29"
    const goToRust = `/?query=%7E%27*22go*22*2c*20*22rust*22&${groups}`
    const mongoToPostgres = `/?query=~%27mongo*2c*20postgres&${groups}`
    const herokoToKubernetes = `/?query=%7E%27*22heroku*22*2c*20kubernetes&${groups}`
    const WaveEmoji = (
    ...
    return (
        <div className="container mx-auto text-left">
            ...
            <p className="mb-2 dark:text-white">
                I thought it would be useful for future infrastructure migrations to have a dataset of previous migrations to learn from. The initial data was scraped from Hacker News, and accidentally included more migrations than intended (e.g. <Link to={`/table?query=~%27*22seattle*22*2c*20*22california*22&${groups}`} className={linkClasses}>Seattle to California</Link>).
            </p>
            <p className="mb-2 dark:text-white">
                You can see how migrations change over time, filtered by segment, and custom search queries (e.g. <Link to={herokoToKubernetes} className={linkClasses}><pre style={{display: "inline"}}>"heroku", kubernetes</pre></Link>, will perform an exact match on Heroku with a fuzzy match on kubernetes). Some interesting patterns emerge (Rubyists moving to Node.JS and then Go), and some ideas for migrations, including <Link to={`/?query=~%27*22reactjs*22*2c*20*22nextjs*22&${groups}`} className={linkClasses}>ReactJS to NextJS</Link> and <Link to={`/?query=~%27*22netlify*22&${groups}`} className={linkClasses}>Google Kubernetes Engine to Netlify</Link>.
            </p>
            <p className="mb-2 dark:text-white">
